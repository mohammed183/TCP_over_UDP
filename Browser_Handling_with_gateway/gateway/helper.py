...
def Send_POST(serverSocket, address, delimiter, filename, http=False):
    packet_count = 0
...
def create_post(fn, http_req):
    f = open(fn, 'w')
    boundary = '----WebKitFormBoundary'
    boundary2 = 'Content-Disposition: form-data; name="file"'
    start_idx = http_req.find(boundary)
    end_idx = http_req.rfind(boundary)
    file_data = http_req[start_idx:end_idx]
    start_idx = file_data.find(boundary2)
    file_data = file_data[start_idx+len(boundary2):]
    print(file_data)
    f.write(file_data)
    f.close()
